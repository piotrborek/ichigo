...
sealed trait OghmaMarshal[F[_]] extends Marshal[F]:
  def marshalInt(value: Int): Stream[F, Byte] =
    value match
      case x if x >= -128 && x <= 127 => marshalInt8(x)

  private def marshalInt8(value: Int): Stream[F, Byte] =
    val buffer = BufferUtils.bigEndianByteBuffer(OghmaTag.Int8.value, value)
